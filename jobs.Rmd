...
Introduction: In this project, we performed data analysis using R language on job demand across data related field in California State. The reason why we did this project is to help people better understand the requirement and information on data - related job.

From this project, we created our own data set using Excel and incorporated the several variables we needed using glassdoor, Indeed and Linkedin. Furthermoer, we subsetted our data set to create various graphs and function. The graphs are created for visualization and the functions are created for searching. The github is used to create a collaborative repository in order to store the work files that are created in R markdown. In this project, we will get more in depth on the functions and graphs that are created in R markdown file.
...
By completing this project, we are able to understand more in depth about the basic requirements and information about data - related job in the state of California. Judging from the result of analysis, graphs were created to help visualizing the behavior of variables in the data set, and functions were created to help getting the  outputs that matched the conditional inputs.  
