...
Let's just use the top 10 preferred language:

```{r}
top10nl = count_order(North_sub_st$Preferred_lang1, North_sub_st$Preferred_lang2,"decreasing", 10)
top10sl = count_order(South_sub_st$Preferred_lang1, South_sub_st$Preferred_lang2,"decreasing", 10)
top10nl
top10sl
```
...
# Make sure to change the value inside of "" to find the desired companies name that satisfied the conditions
   x = jdata$Company[which(jdata$Preferred_lang1 == "Python")]
   y = jdata$Company[which(jdata$Minimum_exp_years <= 1)]
   z = jdata$Company[which(jdata$Region == "NorthCA")]
...
```{r}
find_company = function(Standing, Exp, Region, Language){
