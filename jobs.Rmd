...
if(count(jdata$City)$freq[which(count(jdata$City)$x == (jdata$City[i]))] > 25){ 
  ...
  jdata$City_Bonus[i] = round(rnorm(1, mean = result, sd = 500),1) * 2
  ...
  jdata$City_Bonus[i] = round(rnorm(1, mean = result, sd = 500),1) * 1.75
} else {
  jdata$Jobs_density[i] = "Low"
  jdata$City_Bonus[i] = round(rnorm(1, mean = result, sd = 500),1) * 1.25
}
