...
count(rep(0,length(jdata$Company[which(jdata$Preferred_lang1 == "Python")])))


length(jdata$Company[which(jdata$Preferred_lang1 == "Python")])
...
```{r}
jdata$Company[which(jdata$Preferred_lang1 == "Python")]

count(rep(0,length(jdata$Preferred_lang1 == "Python"))
    
count(rep(0,length(jdata$Preferred_lang1 == "Python")))$freq


jdata$Company[which(jdata$Preferred_lang1 == "SQL") || which( jdata$Preferred_lang2 == "R")]

if(jdata$Preferred_lang1 == "SQL"||jdata$Preferred_lang2 == "R"){
  return(jdata$Company[which(jdata$Preferred_lang1 == "SQL") || which( jdata$Preferred_lang2 == "R")]
         
x = jdata$Company[which(jdata$Preferred_lang1 == "Python")]
y = 
data.frame(x,y,z,u)
...
i = 5
jdata$Company[which(jdata$Minimum_exp_years <= i)]
...
```{r}
n_bach = length(str_subset(North_sub_st$Standing, pattern = "^B")) + count(North_sub_st$Standing)$freq[which(count(North_sub_st$Standing) == "All levels")]  
n_bach

n_master = length(str_subset(North_sub_st$Standing, pattern = "^MS")) + length(str_subset(North_sub_st$Standing, pattern = "MS$")) + count(North_sub_st$Standing)$freq[which(count(North_sub_st$Standing) == "All levels")] - length(str_subset(North_sub_st$Standing, pattern = "^MS$"))
n_master

n_PH.D = length(str_subset(North_sub_st$standing, pattern = "^PH.D")) + length(str_subset(North_sub_st$Standing, pattern = "PH.D$")) + count(North_sub_st$Standing)$freq[which(count(North_sub_st$Standing) == "All levels")]
n_PH.D
```
...
find_company = function(City, Experience, Region, Language){ 

  if (jdata$Standing == Standing){
    return(jdata$Company)
  } else if (jdata$Minimum_exp_years == Experience){
      return(jdata$Standing == Standing)
  } else if (jdata$Region == Region){
      return(jdata$Company)
  } else if(jdata$Preferred_lang1 == Preferred_language){
      return(jdata$Company)
  }
<<<<<<< HEAD
}
...
>>>>>>> 8544f4ea45f0750a0b6362e699959b094efa8d90
```

```{r}
find_company = function(Standing,Experience,Preferred_language) { 
  rep(0,jdata$Standing) for (i in 1:jdata$Standing) {
   if (Standing = jdata$Standing)
     return ( )
  }
}
```
