...
<<<<<<< HEAD
  if(order == "increasing"){
    ...
    result  = data.frame(Faction1 = ct1_pre$x,
                          Freqq1   = ct1_pre$freq)
                          
  }
  return(head(result,top))
=======
  if(order == "decreasing"){
...
>>>>>>> f58919eebba829e79fd6fa011d6d0a45043a8891
}
...
# Names of Companies that satisfy the conditions
   u = c("Python") 
   x = jdata$Company[which(jdata$Preferred_lang1 == u)]
   i = c("1")
   y = jdata$Company[which(jdata$Minimum_exp_years <= i)]
   t = c("North CA")
   z = jdata$Company[which(jdata$Region == t)]

Companies_List = list(x,y,z)
