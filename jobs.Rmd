...
count_order(South_sub_st$Preferred_lang1, South_sub_st$Preferred_lang2,"decreasing", 10)
count_order(North_sub_st$Preferred_lang1, North_sub_st$Preferred_lang2,"decreasing", 10)
...
```{r}
order_lang_S1_pre = count(North_sub_st$Preferred_lang1)
order_lang_S1     = order_lang_S1_pre[order(order_lang_S1_pre$freq, decreasing = T),]
order_lang_S2_pre = count(North_sub_st$Preferred_lang2)
order_lang_S2     = order_lang_S2_pre[order(order_lang_S2_pre$freq, decreasing = T),]

cl = length(order_lang_S1$freq) - length(order_lang_S2$freq)

if(cl < 0){
  
f1result = data.frame(Faction1 = c(as.character(order_lang_S1$x), rep(NA,abs(cl))),
                      Freqq1   = c(order_lang_S1$freq, rep(NA,abs(cl))),
                      Faction2 = as.character(order_lang_S2$x),
                      Freqq2   = order_lang_S2$freq)                  

} else{
   f1result = data.frame(Faction1 = as.character(order_lang_S1$x),
                       Freqq1   = order_lang_S1$freq,
                       Faction2 = c(as.character(order_lang_S2$x), rep(NA,cl)),
                       Freqq2   = c(order_lang_S2$freq,rep(NA,cl)))         
}
...
for(i in 1:20){
 for(j in 1:20){
   if(f1result$Faction1[i] == f1result$Faction2[j]){
      f1result$Freqq1[i] = f1result$Freqq1[i] + f1result$Freqq2[j]
   }
 }
}
f1result[1:10,1:2]
...
find_company = function(preferred_Language,City,Minimum_Experience,Region){
  preferred_Language = a
   x = jdata$Company[which(jdata$Preferred_lang1 == a)]
  City = b
   y = jdata$Company[which(jdata$City == b)]
  Minimum_Experience = c 
   z = jdata$Company[which(jdata$Minimum_exp_years == c)]
  Region = e
   u = jdata$Company[which(jdata$Region == e)]
  
return(list(x,y,z,u))
  
}

find_company("Python","Irvine",3,"NorthCA")
